...
bool is_safe_to_swap() {
  ...
  char tmp_dest[57] = {0};
  parsed_contract_to_string(tmp_dest, sizeof(tmp_dest),
  ...
  if (op->signing.originated != 0) {
    ...
    PRINTF("Amounts differ: expected %d, got %d\n", op->operation.amount,
           swap_values.amount);
    return false;
  } else if (strncmp((const char *)&tmp_dest, swap_values.destination,
                     sizeof(tmp_dest))) {
    PRINTF("Addresses differ: expected %.*H, got %.*H\n", sizeof(tmp_dest),
           tmp_dest, sizeof(tmp_dest), swap_values.destination);
    return false;
  }
  return true;
}
