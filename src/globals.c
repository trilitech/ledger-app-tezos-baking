...
// These are strange variables that the SDK relies on us to define but uses
// directly itself.
ux_state_t G_ux;
...
void clear_apdu_globals(void) { memset(&global.apdu, 0, sizeof(global.apdu)); }
...
void copy_chain(char *out, size_t out_size, void *data) {
  ...
  if (chain_id->v == 0) {
    ...
    chain_id_to_string_with_aliases(out, out_size,
...
void copy_key(char *out, size_t out_size, void *data) {
  ...
  if (baking_key->bip32_path.length == 0) {
    ...
    pubkey_to_pkh_string(out, out_size,
                         (derivation_type_t const)baking_key->derivation_type,
                         &pubkey);
  }
}
