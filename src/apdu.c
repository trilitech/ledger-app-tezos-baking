  ...
  while (true) {
    BEGIN_TRY {
      TRY {
        ...
        // The amount of bytes we get in our APDU must match what the APDU
        // declares its own content length is. All these values are unsigned, so
        // this implies that if rx < OFFSET_CDATA it also throws.
      ...
      CATCH_OTHER(e) {
        clear_apdu_globals(); // IMPORTANT: Application state must not persist
                              // through errors
