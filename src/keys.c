...
size_t read_bip32_path(bip32_path_t *const out, uint8_t const *const in,
  ...
  size_t ix = 0;
  out->length =
      CONSUME_UNALIGNED_BIG_ENDIAN(ix, uint8_t, &buf_as_bip32->length);
  ...
  for (size_t j = 0; j < out->length; j++) {
    out->components[j] = CONSUME_UNALIGNED_BIG_ENDIAN(
        ix, uint32_t, &buf_as_bip32->components[j]);
  }
...
int crypto_derive_private_key(cx_ecfp_private_key_t *private_key,
  ...
  cx_curve_t const cx_curve = signature_type_to_cx_curve(
      derivation_type_to_signature_type(derivation_type));

  if (derivation_type == DERIVATION_TYPE_ED25519) {
    // Old, non BIP32_Ed25519 way...
    error = os_derive_bip32_with_seed_no_throw(
        HDW_ED25519_SLIP10, CX_CURVE_Ed25519, bip32_path->components,
        bip32_path->length, raw_private_key, NULL, NULL, 0);
  } else {
    // derive the seed with bip32_path
    error = os_derive_bip32_no_throw(cx_curve, bip32_path->components,
                                     bip32_path->length, raw_private_key, NULL);
  }

  if (!error)
    // new private_key from raw
    error = cx_ecfp_init_private_key_no_throw(cx_curve, raw_private_key, 32,
                                              private_key);
...
int crypto_init_public_key(derivation_type_t const derivation_type,
  ...
  if (cx_curve == CX_CURVE_Ed25519) {
    error = cx_edwards_compress_point_no_throw(CX_CURVE_Ed25519, public_key->W,
                                               public_key->W_len);
    public_key->W_len = 33;
  }
...
// The caller should not forget to bzero out the `key_pair` as it contains
...
int generate_key_pair(key_pair_t *key_pair,
  ...
  error = crypto_derive_private_key(&key_pair->private_key, derivation_type,
                                    bip32_path);
  if (error) {
    return error;
  }
  // generate corresponding public key
  error = crypto_init_public_key(derivation_type, &key_pair->private_key,
                                 &key_pair->public_key);
  return error;
}
...
void public_key_hash(uint8_t *const hash_out, size_t const hash_out_size,
  ...
  CX_THROW(cx_hash_no_throw((cx_hash_t *)&hash_state, CX_LAST, compressed.W,
...
size_t sign(uint8_t *const out, size_t const out_size,
            derivation_type_t const derivation_type,
            key_pair_t const *const pair, uint8_t const *const in,
            size_t const in_size) {
  check_null(out);
  check_null(pair);
  check_null(in);
  ...
  case SIGNATURE_TYPE_ED25519: {
    ...
    CX_THROW(cx_eddsa_sign_no_throw(&pair->private_key, CX_SHA512,
                                    (uint8_t const *)PIC(in), in_size, out,
                                    SIG_SIZE));
  ...
  case SIGNATURE_TYPE_SECP256R1: {
    ...
    CX_THROW(cx_ecdsa_sign_no_throw(
        &pair->private_key, CX_LAST | CX_RND_RFC6979,
        CX_SHA256, // historical reasons...semantically CX_NONE
        (uint8_t const *)PIC(in), in_size, out, &sig_len, &info));
    tx += sig_len;
