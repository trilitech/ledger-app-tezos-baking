...
// Argument is to distinguish between different parse errors for debugging
// purposes only
...
static inline void
compute_pkh(cx_ecfp_public_key_t *const compressed_pubkey_out,
  ...
  public_key_hash(contract_out->hash, sizeof(contract_out->hash),
                  compressed_pubkey_out, derivation_type, &pubkey);
  contract_out->signature_type =
      derivation_type_to_signature_type(derivation_type);
...
static inline void parse_implicit(
  ...
  out->originated = 0;
  out->signature_type =
      parse_raw_tezos_header_signature_type(raw_signature_type);
  memcpy(out->hash, hash, sizeof(out->hash));
}

static inline void parse_contract(parsed_contract_t *const out,
                                  struct contract const *const in) {
  out->originated = in->originated;
  if (out->originated == 0) { // implicit
    out->signature_type =
        parse_raw_tezos_header_signature_type(&in->u.implicit.signature_type);
...
// Subparsers: no function here should be called anywhere in this file without
// using the CALL_SUBPARSER macro above. Verify this with
// /<funcname>\s*(
...
static inline bool parse_z(uint8_t current_byte,
                           struct int_subparser_state *state, uint32_t lineno) {
...
#define PARSE_Z_MICHELSON                                                      \
  ({                                                                           \
    CALL_SUBPARSER(parse_z_michelson, (byte),                                  \
                   (&state->subparser_state.integer));                         \
    state->subparser_state.integer.value;                                      \
  })

static inline bool parse_next_type(uint8_t current_byte,
                                   struct nexttype_subparser_state *state,
                                   uint32_t sizeof_type, uint32_t lineno) {
#ifdef DEBUG
  if (sizeof_type > sizeof(state->body))
    PARSE_ERROR(); // Shouldn't happen, but error if it does and we're
                   // debugging. Neither side is dynamic.
#endif
...
#define NEXT_TYPE(type)                                                        \
  ({                                                                           \
    CALL_SUBPARSER(parse_next_type, byte, &(state->subparser_state.nexttype),  \
                   sizeof(type));                                              \
...
static inline bool michelson_read_length(uint8_t current_byte,
                                         ...
                                         uint32_t lineno) {
  CALL_SUBPARSER_LN(
      parse_next_type, lineno, current_byte, state,
...
#define MICHELSON_READ_LENGTH                                                  \
  ({                                                                           \
    CALL_SUBPARSER(michelson_read_length, byte,                                \
                   &state->subparser_state.nexttype);                          \
...
static inline bool michelson_read_short(uint8_t current_byte,
                                        struct nexttype_subparser_state *state,
                                        uint32_t lineno) {
  CALL_SUBPARSER_LN(parse_next_type, lineno, current_byte, state,
                    sizeof(uint16_t));
...
#define MICHELSON_READ_SHORT                                                   \
  ({                                                                           \
    CALL_SUBPARSER(michelson_read_short, byte,                                 \
                   &state->subparser_state.nexttype);                          \
    state->subparser_state.nexttype.body.i16;                                  \
  })

static inline bool michelson_read_address(
    uint8_t byte, parsed_contract_t *const out, char *base58_address_buffer,
    struct michelson_address_subparser_state *state, uint32_t lineno) {
  if (state->lineno != lineno) { // We won't have to use CALL_SUBPARSER_LN
                                 // because we're initializing ourselves here.
  ...
  default: {
    ...
    case MICHELSON_TYPE_BYTE_SEQUENCE: {
      ...
      case 2:
        ...
        CALL_SUBPARSER(parse_next_type, byte, &(state->subsub_state),
                       sizeof(state->signature_type));
        memcpy(&(state->signature_type), &(state->subsub_state.body),
               sizeof(state->signature_type));
      ...
      case 3:
        ...
        memcpy(&(state->key_hash), &(state->subsub_state.body),
               sizeof(state->key_hash));

        parse_implicit(out, &state->signature_type,
    ...
    case MICHELSON_TYPE_STRING: {
      ...
      memcpy(base58_address_buffer, state->subsub_state.body.text_pkh,
             sizeof(state->subsub_state.body.text_pkh));
...
#define MICHELSON_READ_ADDRESS(out, buffer)                                    \
  CALL_SUBPARSER(michelson_read_address, byte, (out), buffer,                  \
...
static inline bool parse_byte(uint8_t byte, struct parse_state *const state,
                              struct parsed_operation_group *const out,
                              is_operation_allowed_t is_operation_allowed) {
// OP_STEP finishes the current state transition, setting the state, and
// introduces the next state. For linear chains of states, this keeps the code
// structurally similar to equivalent imperative parsing code.
#define OP_STEP                                                                \
  state->op_step = __LINE__;                                                   \
  return true;                                                                 \
  case __LINE__:

// The same as OP_STEP, but with a particular name, such that we could jump to
// this state.
...
// Shortcuts for defining literal-matching states; mostly used for contract-call
// boilerplate.
...
#define OP_STEP_REQUIRE_LENGTH(constant)                                       \
  ...
  case 0: {
    ...
    const struct operation_group_header *ogh =
    ...
    if (!is_operation_allowed(state->tag))
      PARSE_ERROR();
    ...
    // Parse 'source'
    switch (state->tag) {
    // Tags that don't have "originated" byte only support tz accounts, not KT
    // or tz.
    ...
    case OPERATION_TAG_BABYLON_TRANSACTION: {
      ...
      out->operation.source.signature_type =
          parse_raw_tezos_header_signature_type(
              &implicit_source->signature_type);
      memcpy(out->operation.source.hash, implicit_source->pkh,
    ...
    OP_JMPIF(STEP_AFTER_MANAGER_FIELDS, (state->tag == OPERATION_TAG_PROPOSAL ||
                                         state->tag == OPERATION_TAG_BALLOT));
    ...
    // We don't much care about reveals, they have very little in the way of bad
    // security implications and any fees have already been accounted for
    {
...

    {
      ...
      if (memcmp(out->public_key.W, &(state->subparser_state.nexttype.body.raw),
                 klen) != 0)
        PARSE_ERROR();
  ...
  default:
    ...
    case OPERATION_TAG_PROPOSAL:
      ...
      case STEP_OP_TYPE_DISPATCH: {
        ...
        memcpy(out->operation.proposal.protocol_hash, proposal_data->hash,
    ...
    case OPERATION_TAG_BALLOT:
      ...
      case STEP_OP_TYPE_DISPATCH: {
        ...
        memcpy(out->operation.ballot.protocol_hash, ballot_data->proposal,
    ...
    case OPERATION_TAG_BABYLON_DELEGATION:
      ...
      case STEP_HAS_DELEGATE: {
        ...
        parse_implicit(&out->operation.destination, &dlg->signature_type,
                       dlg->hash);
      }
        JMP_TO_TOP; // These go back to the top to catch any reveals.
      }
    case OPERATION_TAG_ATHENS_ORIGINATION:
    case OPERATION_TAG_BABYLON_ORIGINATION:
      PARSE_ERROR(); // We can't parse the script yet, and all babylon
                     // originations have a script; we have to just reject
                     // originations.
    ...
    case OPERATION_TAG_BABYLON_TRANSACTION:
      ...
      case STEP_OP_TYPE_DISPATCH:
        ...
        OP_STEP {
          ...
          memcpy(&out->operation.implicit_account, &out->operation.source,
                 sizeof(parsed_contract_t));
          memcpy(&out->operation.source, &out->operation.destination,
                 sizeof(parsed_contract_t));
        ...
        OP_STEP { state->argument_length = MICHELSON_READ_LENGTH; }
      ...
      case STEP_MICHELSON_CONTRACT_TO_IMPLICIT_CHAIN:
...

        {
          ...
          if (contractOrImplicit != 0)
            PARSE_ERROR(); // what is this?
        }
        ...
        {
          uint16_t val = MICHELSON_READ_SHORT;
          if (val != MICHELSON_TRANSFER_TOKENS)
            PARSE_ERROR();
...
static void parse_operations_throws_parse_error(
    struct parsed_operation_group *const out, void const *const data,
    size_t length, derivation_type_t derivation_type,
    bip32_path_t const *const bip32_path,
    is_operation_allowed_t is_operation_allowed) {
  size_t ix = 0;
...
bool parse_operations(struct parsed_operation_group *const out,
                      uint8_t const *const data, size_t length,
                      derivation_type_t derivation_type,
                      bip32_path_t const *const bip32_path,
                      is_operation_allowed_t is_operation_allowed) {
  BEGIN_TRY {
    TRY {
      parse_operations_throws_parse_error(out, data, length, derivation_type,
                                          bip32_path, is_operation_allowed);
    }
    CATCH(EXC_PARSE_ERROR) { return false; }
