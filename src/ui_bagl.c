...
void push_ui_callback(char *title, string_generation_callback cb, void *data) {
  ...
  struct screen_data *fmt =
      &global.dynamic_display
           .screen_stack[global.dynamic_display.formatter_index];
...
UX_STEP_NOCB(ux_variable_display, bnnn_paging,
             {
...
UX_STEP_NOCB(ux_app_is_ready_step, nn,
             {
                 "Application",
                 "is ready",
             });

UX_STEP_CB(ux_idle_quit_step, pb, exit_app(),
           {
               &C_icon_dashboard_x,
               "Quit",
           });

UX_FLOW(ux_idle_flow, &ux_app_is_ready_step,

        &ux_init_upper_border, &ux_variable_display, &ux_init_lower_border,

        &ux_idle_quit_step, FLOW_LOOP);
...
UX_STEP_CB(ux_prompt_flow_accept_step, pb, prompt_response(true),
           {&C_icon_validate_14, "Accept"});

UX_STEP_CB(ux_prompt_flow_reject_step, pb, prompt_response(false),
           {&C_icon_crossmark, "Reject"});

UX_STEP_NOCB(ux_eye_step, nn,
             {
                 "Review",
                 "Request",
             });

UX_FLOW(ux_confirm_flow, &ux_eye_step,

        &ux_init_upper_border, &ux_variable_display, &ux_init_lower_border,

        &ux_prompt_flow_reject_step, &ux_prompt_flow_accept_step);
...
// `G_display.formatter_index`. Fills the `screen_title` by copying the `.title`
// field and fills the `screen_value` by computing `callback_fn` with the
// `.data` field as a parameter
void set_screen_data() {
  ...
  clear_data();
  copy_string((char *)G_display.screen_title, sizeof(G_display.screen_title),
              fmt->title);
  fmt->callback_fn(G_display.screen_value, sizeof(G_display.screen_value),
                   fmt->data);
}
...
void display_next_state(bool is_left_ux_step) {
  if (is_left_ux_step) { // We're called from the LEFT ux step
    if (G_display.current_state == STATIC_SCREEN) {
      // The previous screen was a static screen, so we're now entering the
      // stack (from the start of the stack).
      ...
      // The previous screen was NOT a static screen, so we were already in the
      // stack.
      if (G_display.formatter_index == 0) {
        ...
        // Don't need to update the screen data, simply display the
        // ux_flow_prev() which will be a static screen.
        ux_flow_prev();
      } else {
        // The user is still browsing the stack: simply decrement
        // `formatter_index` and update `screen_data`.
    ...
    if (G_display.current_state == STATIC_SCREEN) {
      // We're called from a static screen, so we're now entering the stack
      ...
      // We're being called from a dynamic screen, so the user was already
      // browsing the stack. The user wants to go to the next screen so
      // increment the index.
      G_display.formatter_index++;
      if (G_display.formatter_index == G_display.screen_stack_size) {
        // Index is equal to stack size, so time to exit the stack and display
        // the static screens.
