...
bool is_valid_level(level_t lvl) { return !(lvl & 0xC0000000); }
...
void write_high_water_mark(parsed_baking_data_t const *const in) {
  ...
  UPDATE_NVRAM(ram, {
    // If the chain matches the main chain *or* the main chain is not set, then
    // use 'main' HWM.
    high_watermark_t volatile *const dest =
        select_hwm_by_chain(in->chain_id, ram);
    if ((in->level > dest->highest_level) ||
        (in->round > dest->highest_round)) {
    ...
    dest->had_endorsement |= (in->type == BAKING_TYPE_ENDORSEMENT ||
                              in->type == BAKING_TYPE_TENDERBAKE_ENDORSEMENT);
    dest->had_preendorsement |=
        in->type == BAKING_TYPE_TENDERBAKE_PREENDORSEMENT;
...
void authorize_baking(derivation_type_t const derivation_type,
  ...
  if (bip32_path->length >
          NUM_ELEMENTS(N_data.baking_key.bip32_path.components) ||
      bip32_path->length == 0)
    return;
...
static bool is_level_authorized(parsed_baking_data_t const *const baking_info) {
  ...
  if (baking_info->is_tenderbake) {
    return baking_info->level > hwm->highest_level ||
           ...
           // It is ok to sign an endorsement if we have not already signed an
           // endorsement for the level/round
           (baking_info->level == hwm->highest_level &&
            baking_info->round == hwm->highest_round &&
            baking_info->type == BAKING_TYPE_TENDERBAKE_ENDORSEMENT &&
            !hwm->had_endorsement) ||

           // It is ok to sign a preendorsement if we have not already signed
           // neither an endorsement nor a preendorsement for the level/round
           (baking_info->level == hwm->highest_level &&
            baking_info->round == hwm->highest_round &&
    ...
    return baking_info->level > hwm->highest_level
           ...
           // endorsement, and we must not have previously seen an endorsement.
           || (baking_info->level == hwm->highest_level &&
               baking_info->type == BAKING_TYPE_ENDORSEMENT &&
               !hwm->had_endorsement);
  }
}
...
bool is_path_authorized(derivation_type_t const derivation_type,
                        bip32_path_t const *const bip32_path) {
  check_null(bip32_path);
  return derivation_type != 0 &&
         derivation_type == N_data.baking_key.derivation_type &&
         bip32_path->length > 0 &&
         bip32_paths_eq(bip32_path,
...
bool parse_block(parsed_baking_data_t *const out, void const *const data,
                 size_t const length) {
  if (length < sizeof(struct block_wire) + EMMY_FITNESS_SIZE)
    return false;
  ...
  case 2: // Tenderbake
    ...
    uint32_t fitness_size =
        READ_UNALIGNED_BIG_ENDIAN(uint32_t, &block->fitness_size);
    if (fitness_size < (4 + sizeof(uint32_t)) ||
        fitness + fitness_size > data + length ||
        fitness_size > MAXIMUM_TENDERBAKE_FITNESS_SIZE) {
      return false;
    }
    out->round =
        READ_UNALIGNED_BIG_ENDIAN(uint32_t, (fitness + fitness_size - 4));
    return true;
  default:
    return false;
  }
...
bool parse_consensus_operation(parsed_baking_data_t *const out,
  ...
  case 23: // tenderbake endorsement + DAL
    if (length < sizeof(struct tenderbake_consensus_op_wire))
      return false;
