...
void public_key_hash(
    ...
    cx_ecfp_public_key_t
        *const compressed_out, // pass NULL if this value is not desired
    derivation_type_t const derivation_type,
    cx_ecfp_public_key_t const *const restrict public_key);

size_t sign(uint8_t *const out, size_t const out_size,
            derivation_type_t const derivation_type,
            key_pair_t const *const key, uint8_t const *const in,
            size_t const in_size);

// Read a curve code from wire-format and parse into `deviration_type`.
static inline derivation_type_t
parse_derivation_type(uint8_t const curve_code) {
  switch (curve_code) {
  case 0:
...
unparse_derivation_type(derivation_type_t const derivation_type) {
...
static inline signature_type_t
derivation_type_to_signature_type(derivation_type_t const derivation_type) {
  switch (derivation_type) {
  case DERIVATION_TYPE_SECP256K1:
...
static inline cx_curve_t
signature_type_to_cx_curve(signature_type_t const signature_type) {
