...
static inline size_t hmac(uint8_t *const out, size_t const out_size,
  ...
  static uint8_t const key_sha256[] = {
      0x6c, 0x4e, 0x7e, 0x70, 0x6c, 0x54, 0xd3, 0x67, 0xc8, 0x7a, 0x8d,
      0x89, 0xc1, 0x6a, 0xdf, 0xe0, 0x6c, 0xb5, 0x68, 0x0c, 0xb7, 0xd1,
      0x8e, 0x62, 0x5a, 0x90, 0x47, 0x5e, 0xc0, 0xdb, 0xdb, 0x9f};

  // Deterministically sign the SHA256 value to get something directly tied to
  // the secret key.
  ...
  BEGIN_TRY {
    TRY {
      generate_key_pair(&key_pair, derivation_type,
                        &global.path_with_curve.bip32_path);
      signed_hmac_key_size =
          sign(state->signed_hmac_key, sizeof(state->signed_hmac_key),
               derivation_type, &key_pair, key_sha256, sizeof(key_sha256));
    }
...
size_t handle_apdu_hmac(__attribute__((unused)) uint8_t instruction,
  ...
  size_t const hmac_size = hmac(G.hmac, sizeof(G.hmac), &G, data_to_hmac,
                                data_to_hmac_size, derivation_type);
