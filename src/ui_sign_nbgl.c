...
static void cancel_callback(void) {
  nbgl_useCaseStatus(transactionContext.cancelled_status, false,
                     ui_initial_screen);
  transactionContext.cxl_cb();
}

static void approve_callback(void) {
  nbgl_useCaseStatus(transactionContext.confirmed_status, true,
                     ui_initial_screen);
...
void prompt_register_delegate(ui_callback_t const ok_cb,
  ...
  nbgl_useCaseReviewStart(&C_tezos, "Register delegate", NULL, "Cancel",
...
static void prompt_cancel(void) {
  nbgl_useCaseConfirm("Reject transaction?", NULL, "Yes, Reject",
                      "Go back to transaction", cancel_callback);
}
...
#define MAX_NUMBER_CHARS                                                       \
  (MAX_INT_DIGITS + 2) // include decimal point and terminating null
bool prompt_transaction(struct parsed_operation_group const *const ops,
  ...
  if (called_from_swap) {
    ...
    if (valid) {
      // We're called from swap and we've verified that the data is correct.
      // Sign it.
      ok();
    } else {
  ...
  case OPERATION_TAG_PROPOSAL: {
    ...
    nbgl_useCaseReviewStart(&C_tezos, "Review proposal", NULL, "Cancel",
                            continue_light_callback, cancel_callback);
    break;
  }
  ...
  case OPERATION_TAG_BALLOT: {
    ...
    nbgl_useCaseReviewStart(&C_tezos, "Review vote", NULL, "Cancel",
                            continue_light_callback, cancel_callback);
    break;
  }
  ...
  case OPERATION_TAG_BABYLON_ORIGINATION: {
    ...
    nbgl_useCaseReviewStart(&C_tezos, "Review origination", NULL, "Cancel",
                            continue_light_callback, cancel_callback);
    break;
  }
  ...
  case OPERATION_TAG_BABYLON_DELEGATION: {
    ...
    nbgl_useCaseReviewStart(&C_tezos, type_msg, NULL, "Cancel",
                            continue_light_callback, cancel_callback);
    break;
  }
  ...
  case OPERATION_TAG_BABYLON_TRANSACTION: {
    ...
    nbgl_useCaseReviewStart(&C_tezos, "Review Transaction", NULL, "Cancel",
                            continue_callback, prompt_cancel);
    break;
  }
  case OPERATION_TAG_NONE: {
    ...
    nbgl_useCaseReviewStart(&C_tezos, "Reveal key to\nBlockchain", NULL,
                            "Cancel", continue_light_callback, cancel_callback);
    break;
  }
  }
  return 0;
...
size_t wallet_sign_complete(uint8_t instruction, uint8_t magic_byte,
  ...
  if (instruction == INS_SIGN_UNSAFE) {
    ...
    nbgl_useCaseReviewStart(&C_tezos, "Review pre-hashed", ops, "Cancel",
                            continue_light_callback, cancel_callback);
    *flags = IO_ASYNCH_REPLY;
    return 0;
  } else {
    ui_callback_t const ok_c = instruction == INS_SIGN_WITH_HASH
                                   ? sign_with_hash_ok
                                   : sign_without_hash_ok;
    ...
    case MAGIC_BYTE_UNSAFE_OP:
      if (!G.maybe_ops.is_valid ||
          prompt_transaction(&G.maybe_ops.v, &global.path_with_curve, ok_c,
                             sign_reject)) {
        goto unsafe;
      }
      *flags = IO_ASYNCH_REPLY;
  ...
  unsafe:
    ...
    if (magic_byte == MAGIC_BYTE_UNSAFE_OP3) {
      ...
      transactionContext.infoLongPress.text =
    ...
    nbgl_useCaseReviewStart(&C_tezos, ops, NULL, "Cancel",
                            continue_light_callback, cancel_callback);
