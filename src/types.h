...
//      is almost the same as `signature_type` but allows for multiple
//      derivation strategies for ed25519. This type is often parsed from the
//      APDU instruction. See `parse_derivation_type` for the mapping.
...
// Uses K&R style declaration to avoid being stuck on const void *, to avoid
// having to cast the function pointers.
...
static inline void copy_bip32_path(bip32_path_t *const out,
                                   bip32_path_t volatile const *const in) {
  check_null(out);
  check_null(in);
  memcpy(out->components, (void *)in->components,
         in->length * sizeof(*in->components));
...
static inline bool bip32_paths_eq(bip32_path_t volatile const *const a,
  ...
  return a == b ||
         (a != NULL && b != NULL && a->length == b->length &&
          memcmp((void const *)a->components, (void const *)b->components,
...
static inline void
copy_bip32_path_with_curve(bip32_path_with_curve_t *const out,
...
static inline bool
bip32_path_with_curve_eq(bip32_path_with_curve_t volatile const *const a,
...
// Macros to wrap a static prompt and value strings and ensure they aren't too
// long.
#define PROMPT(str)                                                            \
  ({                                                                           \
...
#define STATIC_UI_VALUE(str)                                                   \
  ({                                                                           \
    _Static_assert(sizeof(str) <= VALUE_WIDTH + 1 /*null byte*/,               \
                   str " won't fit in the UI.");                               \
    str;                                                                       \
  })
...
typedef struct parsed_contract {
  ...
  signature_type_t signature_type; // 0 in originated case
                                   // An implicit contract with signature_type
                                   // of 0 means not present
...
enum operation_tag {
  OPERATION_TAG_NONE =
      -1, // Sentinal value, as 0 is possibly used for something
