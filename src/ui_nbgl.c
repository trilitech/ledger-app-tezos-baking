...
static const char *const infoContents[] = {VERSION, "Ledger",
                                           "(c) 2023 Ledger"};

void ui_initial_screen(void) { ux_idle_screen(NULL, NULL); }
...
void ui_menu_about_baking(void) {
  nbgl_useCaseSettings("Tezos baking", 0, 2, false, ui_initial_screen,
                       navigation_cb_baking, NULL);
}
...
void ui_menu_about_wallet(void) {
  nbgl_useCaseSettings("Tezos wallet", 0, 1, false, ui_initial_screen,
                       navigation_cb_wallet, NULL);
}
...
#ifdef BAKING_APP
  nbgl_useCaseHome("Tezos Baking", &C_tezos, NULL, false, ui_menu_about_baking,
                   exit_app);
#else
