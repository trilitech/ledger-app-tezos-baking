...
// Zeros out all application-specific globals and SDK-specific UI/exchange
// buffers.
void init_globals(void);

#define MAX_APDU_SIZE 235 // Maximum number of bytes in a single APDU

// Our buffer must accommodate any remainder from hashing and the next message
// at once.
...
typedef struct {
  uint8_t packet_index; // 0-index is the initial setup packet, 1 is first
                        // packet to hash, etc.
...
#define UPDATE_NVRAM(out_name, body)                                           \
  ({                                                                           \
    ...
    body;                                                                      \
    nvm_write((void *)&N_data, &global.apdu.baking_auth.new_data,              \
              sizeof(N_data));                                                 \
