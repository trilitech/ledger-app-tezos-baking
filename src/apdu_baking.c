...
size_t handle_apdu_all_hwm(__attribute__((unused)) uint8_t instruction,
  ...
  int has_a_chain_migrated = N_data.hwm.main.migrated_to_tenderbake ||
...
size_t
handle_apdu_query_auth_key(__attribute__((unused)) uint8_t instruction,
                           __attribute__((unused)) volatile uint32_t *flags) {
...
size_t handle_apdu_query_auth_key_with_curve(
  ...
  size_t tx = 0;
  G_io_apdu_buffer[tx++] =
      unparse_derivation_type(N_data.baking_key.derivation_type);
  G_io_apdu_buffer[tx++] = length;
  for (uint8_t i = 0; i < length; ++i) {
    tx = send_word_big_endian(tx, N_data.baking_key.bip32_path.components[i]);
  }
...
size_t
handle_apdu_deauthorize(__attribute__((unused)) uint8_t instruction,
                        __attribute__((unused)) volatile uint32_t *flags) {
