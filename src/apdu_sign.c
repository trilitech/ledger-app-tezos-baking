...
static inline void
conditional_init_hash_state(blake2b_hash_state_t *const state) {
  check_null(state);
...
static void blake2b_incremental_hash(
  ...
  while (*out_length > B2B_BLOCKBYTES) {
    ...
    CX_THROW(cx_hash_no_throw((cx_hash_t *)&state->state, 0, current,
                              B2B_BLOCKBYTES, NULL, 0));
...
static void blake2b_finish_hash(
  ...
  CX_THROW(cx_hash_no_throw((cx_hash_t *)&state->state, CX_LAST, buff,
                            *buff_length, out, out_size));
}

static inline void clear_data(void) { memset(&G, 0, sizeof(G)); }
...
static bool parse_allowed_operations(struct parsed_operation_group *const out,
  ...
  return parse_operations(out, in, in_size, key->derivation_type,
                          &key->bip32_path, &is_operation_allowed);
}

#else

static bool
parse_allowed_operation_packet(struct parsed_operation_group *const out,
...
static uint8_t get_magic_byte_or_throw(uint8_t const *const buff,
...
static size_t handle_apdu(bool const enable_hashing, bool const enable_parsing,
                          uint8_t const instruction, volatile uint32_t *flags) {
  ...
  if (enable_parsing) {
    ...
    if (G.magic_byte == MAGIC_BYTE_UNSAFE_OP) {
      ...
      G.maybe_ops.is_valid = parse_allowed_operations(
          &G.maybe_ops.v, buff, buff_size, &global.path_with_curve);
    } else {
    ...
    if (G.packet_index == 1) {
      ...
      // If it is an "operation" (starting with the 0x03 magic byte), set up
      // parsing If it is arbitrary Michelson (starting with 0x05), dont bother
      // parsing and show the "Sign Hash" prompt
      if (G.magic_byte == MAGIC_BYTE_UNSAFE_OP) {
        parse_operations_init(
            &G.maybe_ops.v, global.path_with_curve.derivation_type,
            &global.path_with_curve.bip32_path, &G.parse_state);
      }
  ...
  if (enable_hashing) {
    ...
    blake2b_incremental_hash(G.message_data, sizeof(G.message_data),
                             &G.message_data_length, &G.hash_state);
  }
  ...
  if (last) {
    if (enable_hashing) {
      ...
      blake2b_incremental_hash(G.message_data, sizeof(G.message_data),
                               &G.message_data_length, &G.hash_state);
      blake2b_finish_hash(G.final_hash, sizeof(G.final_hash), G.message_data,
                          sizeof(G.message_data), &G.message_data_length,
                          &G.hash_state);
    }

    G.maybe_ops.is_valid =
        parse_operations_final(&G.parse_state, &G.maybe_ops.v);
...
int perform_signature(bool const on_hash, bool const send_hash) {
  ...
  size_t const data_length =
      on_hash ? sizeof(G.final_hash) : G.message_data_length;

  key_pair_t key_pair = {0};
  size_t signature_size = 0;

  int error =
      generate_key_pair(&key_pair, global.path_with_curve.derivation_type,
  ...
  BEGIN_TRY {
    TRY {
      signature_size = sign(&G_io_apdu_buffer[tx], MAX_SIGNATURE_SIZE,
                            global.path_with_curve.derivation_type, &key_pair,
                            data, data_length);
    }
    CATCH_OTHER(e) { error = e; }
    FINALLY { memset(&key_pair, 0, sizeof(key_pair)); }
  }
  END_TRY;
