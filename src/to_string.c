...
void pkh_to_string(char *const buff, size_t const buff_size,
...
void parsed_contract_to_string(char *const buff, size_t const buff_size,
  ...
  if (contract->hash_ptr != NULL) {
    if (buff_size < HASH_SIZE_B58)
      THROW(EXC_WRONG_LENGTH);
    memcpy(buff, contract->hash_ptr, HASH_SIZE_B58);
  } else if (contract->originated == 0 &&
             contract->signature_type == SIGNATURE_TYPE_UNSET) {
    if (buff_size < sizeof(NO_CONTRACT_STRING))
      THROW(EXC_WRONG_LENGTH);
    strlcpy(buff, NO_CONTRACT_STRING, buff_size);
  } else {
    signature_type_t const signature_type = contract->originated != 0
...
void lookup_parsed_contract_name(char *const buff, size_t const buff_size,
                                 parsed_contract_t const *const contract) {
  parsed_contract_to_string(buff, buff_size, contract);

  for (uint16_t i = 0; i < sizeof(named_delegates) / sizeof(named_delegate_t);
       i++) {
  ...
  if (buff_size <= strlen(NO_CONTRACT_NAME_STRING))
    THROW(EXC_WRONG_LENGTH);
  strlcpy(buff, NO_CONTRACT_NAME_STRING, buff_size);
}

void pubkey_to_pkh_string(char *const out, size_t const out_size,
  ...
  pkh_to_string(out, out_size,
                derivation_type_to_signature_type(derivation_type), hash);
}
...
void bin_to_base58(char *const out, size_t const out_size,
                   uint8_t const *const in, size_t const in_size) {
  check_null(out);
  check_null(in);
  size_t buff_size = out_size;
  if (!b58enc(out, &buff_size, (uint8_t const *)PIC(in), in_size))
    THROW(EXC_WRONG_LENGTH);
}

void buffer_to_base58(char *const out, size_t const out_size,
                      buffer_t const *const in) {
  check_null(out);
  check_null(in);
...
void pkh_to_string(char *const buff, size_t const buff_size,
  ...
  compute_hash_checksum(data.checksum, &data,
...
void protocol_hash_to_string(char *buff, const size_t buff_size,
                             const uint8_t hash[PROTOCOL_HASH_SIZE]) {
  check_null(buff);
  check_null(hash);
  if (buff_size < PROTOCOL_HASH_BASE58_STRING_SIZE)
    THROW(EXC_WRONG_LENGTH);
...
void chain_id_to_string(char *const buff, size_t const buff_size,
  ...
  struct __attribute__((packed)) {
    ...
    uint8_t checksum[TEZOS_HASH_CHECKSUM_SIZE];
            ...
            // Must hash big-endian data so treating little endian as big endian
            // just flips
...
void chain_id_to_string_with_aliases(char *const out, size_t const out_size,
...
// This function fills digits, potentially with all leading zeroes, from the end
// of the buffer backwards This is intended to be used with a temporary buffer
// of length MAX_INT_DIGITS Returns offset of where it stopped filling in
static inline size_t convert_number(char dest[MAX_INT_DIGITS], uint64_t number,
                                    bool leading_zeroes) {
  check_null(dest);
...
void number_to_string_indirect64(char *const dest, size_t const buff_size,
...
void number_to_string_indirect32(char *const dest, size_t const buff_size,
...
void microtez_to_string_indirect(char *const dest, size_t const buff_size,
                                 uint64_t const *const number) {
  check_null(dest);
  check_null(number);
...
int microtez_to_string_indirect_no_throw(char *const dest,
                                         size_t const buff_size,
                                         uint64_t const *const number) {
  if (!dest || !number) {
    return (0);
  }
  if (buff_size < MAX_INT_DIGITS + sizeof(TICKER_WITH_SPACE) +
                      1) { // + terminating null + decimal point
    return (0);
  }
...
void copy_string(char *const dest, size_t const buff_size,
                 char const *const src) {
  check_null(dest);
  check_null(src);
  char const *const src_in = (char const *)PIC(src);
  // I don't care that we will loop through the string twice, latency is not an
  // issue
...
void bin_to_hex(char *const out, size_t const out_size, uint8_t const *const in,
...
void buffer_to_hex(char *const out, size_t const out_size,
                   buffer_t const *const in) {
  check_null(out);
  check_null(in);
  buffer_t const *const src = (buffer_t const *)PIC(in);
